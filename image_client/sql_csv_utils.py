...
def create_timestamps(start_time: datetime, end_time: datetime,
    ...
    sql = create_sql_string(val_list=value_list, col_list=column_list, tab_name="picturae_batch")

    return sql


def create_update_string(tab_name: str, col_list: list, val_list: list, condition: str):
    """create_update_string: function used to create string"""
    update_string = " SET"
...
def create_unmatch_tab(row,df, tab_name: str):
    ...
    sql = create_sql_string(tab_name=tab_name, col_list=col_list,
                            val_list=val_list)
...
def create_new_tax(row, df: pd.DataFrame, tab_name: str):
    columns = df.columns
    fullname = columns.get_loc('fullname')
    catalog_number = columns.get_loc('CatalogNumber')
