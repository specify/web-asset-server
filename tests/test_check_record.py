...
class DatabaseChecks(unittest.TestCase, TestingTools):
    ...
    def test_barcode_present(self):
        ...
        self.TestCsvCreatePicturae.barcode_has_record()
        # checks whether boolean column correctly added
        self.assertEqual(len(self.TestCsvCreatePicturae.record_full.columns), 4)
        # checks that no NAs were dropped
        self.assertEqual(len(self.TestCsvCreatePicturae.record_full), 3)
        # checks that the correct boolean order is returned
        test_list = list(self.TestCsvCreatePicturae.record_full['barcode_present'])
    ...
    def test_if_barcode_match(self):
        ...
        self.TestCsvCreatePicturae.check_barcode_match()
        test_list = list(self.TestCsvCreatePicturae.record_full['is_barcode_match'])
    ...
    def test_image_has_record(self):
        ...
        self.TestCsvCreatePicturae.image_has_record()
        test_list = list(self.TestCsvCreatePicturae.record_full['image_present'])
